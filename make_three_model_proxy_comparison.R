#!/usr/bin/env Rscript
root<-normalizePath(".");out<-file.path(root,"outputs","validation_graphs");dir.create(out,recursive=TRUE,showWarnings=FALSE)
nd<-file.path(root,"external_validation_no_overlap");pd<-file.path(root,"external_validation_proxy_no_overlap")
