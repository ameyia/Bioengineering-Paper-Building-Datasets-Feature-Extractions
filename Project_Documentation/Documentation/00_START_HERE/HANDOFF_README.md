...
- `external_validation_no_overlap_rerun/`: cleaned non-proxy, 10-seed results.
- `external_validation_proxy_no_overlap/`: cleaned proxy-integrated results.
- `outputs/validation_graphs/`: comparison figures generated from the validation
  summaries.
- `s1_proxy_no_overlap_*`: proxy-integrated inputs and overlap audit.

Do not use `external_validation_no_overlap_Test_notActualResults/` for scientific
reporting. Its name marks it as a test/non-final output.

## Important dataset counts

- `pcp_aac.tsv`, the older/base feature file, contains 88 positive and 435 negative
...
3. Review `outputs/validation_graphs/`.
