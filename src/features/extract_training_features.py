...
# -----------------------------
positive_sequences = []
for record in SeqIO.parse("data/raw/TableS1PositiveProteins.fasta", "fasta"):
    seq = str(record.seq).strip().upper()
    positive_sequences.append((seq, 1))

print(f"Loaded {len(positive_sequences)} positive sequences")

neg_df = pd.read_csv("data/raw/final_negatives.tsv", sep="\t", header=None)
negative_sequences = []
...
df_base.to_csv("data/processed/baseline/pcp_aac.tsv", sep="\t", index=False)
df_ctd.to_csv("data/processed/baseline/pcp_aac_ctd.tsv", sep="\t", index=False)
df_dpc.to_csv("data/processed/baseline/pcp_aac_ctd_dpc.tsv", sep="\t", index=False)

print("\nSaved:")
print("- data/processed/baseline/pcp_aac.tsv")
print("- data/processed/baseline/pcp_aac_ctd.tsv")
print("- data/processed/baseline/pcp_aac_ctd_dpc.tsv")
